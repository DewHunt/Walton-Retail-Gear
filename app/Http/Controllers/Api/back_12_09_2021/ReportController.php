...
class ReportController extends Controller
{
    ...
    public function incentiveReportFrom()
    {
        $salesIncentiveReportList = DB::table('view_sales_incentive_reports')
        ->select('id','bp_id','retailer_id','bp_name','retailer_name',\DB::raw('SUM(incentive_amount) AS total_incentive'),\DB::raw('SUM(incentive_sale_qty) AS total_qty'))
        ->where('sale_id','>',0)
        ->get();
    ...
    public function incentiveReport(Request $request)
    {
        ...
        $salesIncentiveReportList = DB::table('view_sales_incentive_reports')
        ->select('id','bp_id','retailer_id','bp_name','retailer_name',\DB::raw('SUM(incentive_amount) AS total_incentive'),'zone_name',\DB::raw('SUM(incentive_sale_qty) AS total_qty'))
    ...
    public function bpAttendanceForm()
    {
    	$month_Sdate    =  date('Y-m-01 00:00:00');
        $month_Edate    =  date('Y-m-t 23:59:59');
        ...
        //echo "<pre>";print_r($attendanceList);echo "</pre>";exit();
    ...
    public function bpLeaveReportForm()
    {
    ...
    public function OrderDetailsView($saleId)
    {
        ...
        foreach($saleProductList as $row) {
            ...
            $viewItemList[] = "<tr><td>".$i.".</td><td><img src=".$pathUrl.$imagePath." width='50' height='50'/></td><td>".$row->ime_number."</td><td>".$row->product_code."</td><td>".$row->product_type."</td><td>".$row->product_model."</td><td>".$row->product_color."</td><td>".number_format($row->msrp_price,2)."</td><td>".number_format($row->sale_price,2)."</td><td>".$row->sale_qty."</td></tr>";
    ...
    public function editLeave($id)
    {
        if(isset($id) && $id > 0) {
            ...
            return response()->json(['leaveInfo'=>$leaveInfo,'leaveType'=>$leaveType,'leaveReason'=>$leaveReason]);
        } else {
    ...
    public function modelSalesReport_old(Request $request)
    {
        $bpId      = 0;
        $retailId  = 0;
        if($request->input('bp_id')) {
    ...
    public function modelSalesReport(Request $request)
    {
        ...
        return view('admin.report.seller_product_sales_report',compact('productSalesReport','productModelList'));
    }
    ...
    public function searchRetailerStock(Request $request)
    {
        ...
        if(empty($dCode) || $dCode == "") {
            $getDealerCode = DB::table('dealer_informations')->where('dealer_phone_number','=',$phone)->first();
    ...
    public function preOrderReportDetails($model)
    {
        ...
        ->where('prebooking_orders.model','=',$model)
        ->whereBetween('booking_date',[$month_Sdate,$month_Edate])
        ->get();
